...
ご希望なら次のどれかを作成します（1つ選んでください）
- A: Releases ページ用の短い「ダウンロード→検証→実行」手順を生成  
- B: CONTRIBUTING.md と Issue テンプレートを作る  
- C: GitHub Actions の実行ログを自動でチェックする簡易 PowerShell スクリプトを作る